...
const Post = () => {
  ...
  const [Type, setType] = useState({id: 0, title: 'Chọn loại'});
  ...
  const logicPost = () => {
    ...
    fetch('http://10.22.208.139:1805/upload', {
      body: formData,
      method: 'post',
    })
      .then(data => data.json())
      .then(data => {
        postData(`${baseURL}Insert.php`, {
          TieuDe: nameProduct,
          Gia: price,
          MoTa: Description,
          TinhTrang: 90,
    ...
    <SafeAreaView
      ...
      <ScrollView
        ...
        <View
          ...
          <Button on_press={handlePost} btn_text={'Đăng bài'} />
        </View>
      </ScrollView>
    </SafeAreaView>
  );
